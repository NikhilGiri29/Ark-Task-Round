...
def neighbour_node(node_mat, Node) -> List: 
    ...
    return arr
def heuristic(alpha : Node):
    global end
    return alpha.distance  + np.sqrt(((alpha.x - end.x)**2) + ((alpha.y - end.y)**2))
...
def A_star(start, end):
    ...
    while len(queue):
        ...
        for obj in (queue):
            if heuristic(obj) < heuristic(currentNode):
                currentNode = obj
...
print("Almost There...")
path = A_star(start, end) #saved the pathfound array as path but have not used it anywhere in this case
